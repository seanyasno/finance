...
must_haves:
  ...
  key_links:
    - from: "TransactionListView.swift"
      ...
      via: "groupingMode parameter"
      pattern: "groupingMode:"
...
1. Add new parameter to control card visibility:
   ```swift
   let showCard: Bool  // Default true, false when grouped by card
   ```
   - Keep default `true` for backward compatibility using default parameter value
...
1. In the ForEach where TransactionRowView is created, pass showCard:
   ```swift
   TransactionRowView(
...
2. When groupingMode is .creditCard, showCard will be false, hiding redundant card info in rows
3. For .date and .month modes, showCard remains true
